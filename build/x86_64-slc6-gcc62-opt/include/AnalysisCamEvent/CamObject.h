         ...
         if(checkStoregate) {
            //have to test d3pdIndex to see if we are trying to get a vector of T or just a T
               if(m_d3pdIndex<0) {
                  ...
                  //DummyObj<std::vector<W>,(ClassID_traits<std::vector<W> >::s_version)> dObj(this);
                  if(DummyObj<std::vector<WnoRef>,(ClassID_traits<std::vector<WnoRef> >::s_version)>::contains(m_prefix+key,m_evtStore)) return STOREGATE;
               }
         }
